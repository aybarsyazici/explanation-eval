...
export const Header: React.FC<HeaderProps> = ({ onTabChange, setDarkMode }) => {
  ...
  const darkModeToggle = useMemo(() => {
    return (
      <div id="header-tabs-switch">
        <Popover content={isDarkMode ? "Light Mode" : "Dark Mode"}>
          <Switch
  ...
  return (
    <Menu
      ...
      mode="horizontal"
    >
      <Menu.Item key="about" id="header-tabs-about" icon={<UserOutlined />}>
        About Us
      </Menu.Item>
      <Menu.Item key="app" id="header-tabs-app" icon={<FormOutlined />}>
        Recipe Improver
      </Menu.Item>
      {darkModeToggle}
    </Menu>
